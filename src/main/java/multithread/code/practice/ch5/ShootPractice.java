...
public class ShootPractice {
    ...
    public ShootPractice(int N, final int lineCount, int lasting) {
        ...
        // 语句②
        startBarrier = new CyclicBarrier(N, new Runnable() {
            @Override
            public void run() {
                System.out.println("start barrier run");
            }
        });
        // todo 定义一下 startBarrier  的  runnable
    }
    ...
    static class Soldier {
        ...
        private void sleepRandomTime(int milli) {
            int sleepTime = (int) (1 + Math.random() * (milli - 1));
            try {
